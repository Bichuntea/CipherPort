...
- Keep the distributable merged firmware below 6.5 MB (6,500,000 bytes),
  within the physical 8 MB Flash; never include user vault contents in it.
- Preserve `cardid` at `0x356000`. New devices use a 4 MB application at
  `0x360000` with the vault at `0x10000`; existing devices retain the 3 MB
  application at `0x10000` and vault at `0x35a000`. Never flash a contiguous
  0x0 image through `cardid`; use segmented factory packages and
  partition-matched, application-only upgrades.
