...
- 可分发的整合固件包须小于 6.5 MB（6,500,000 字节），并处于物理 8 MB Flash 范围内；不得打包用户密码库内容。
- 必须保留 `cardid@0x356000`。新设备使用 `0x360000` 起的 4 MB 应用分区和
  `0x10000` 起的密码库；现有设备维持 `0x10000` 起的 3 MB 应用分区和
  `0x35a000` 起的密码库。禁止从 `0x0` 连续刷写穿过 `cardid`；新设备使用
  分段出厂包，旧设备使用与现有分区匹配的仅应用升级包。
