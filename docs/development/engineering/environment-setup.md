...
isolated temporary builds and produces three verified segmented packages:

```text
build/AI-Passport-factory-4m.zip
build/AI-Passport-upgrade-legacy-3m.zip
build/AI-Passport-upgrade-factory-4m.zip
```
...
Use the package flasher, which checks the device partition table and writes
only the package's explicit segments:

```bash
python tools/flash_package.py build/AI-Passport-factory-4m.zip --port <port>
# Existing devices: substitute AI-Passport-upgrade-legacy-3m.zip.
idf.py -p <port> monitor
```

Never write a merged BIN at `0x0` with the 4 MB layout: the contiguous byte
range crosses `cardid`. Factory packages require a blank partition table;
upgrades use the device's existing 3 MB or 4 MB layout and write only the app.
