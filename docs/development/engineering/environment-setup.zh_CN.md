...
编译方式，它使用隔离的临时构建，并生成三种经验证的分段固件包：
...
使用会先核对设备分区表、只写入包内明确分段的刷写工具：

```bash
python tools/flash_package.py build/AI-Passport-factory-4m.zip --port <port>
# 已使用的旧设备改用 AI-Passport-upgrade-legacy-3m.zip。
idf.py -p <port> monitor
```

4 MB 布局不得把合并 BIN 从 `0x0` 连续写入，否则会穿过 `cardid`。出厂包要求
空白分区表；升级包按设备已有的 3 MB 或 4 MB 布局仅写入应用。
