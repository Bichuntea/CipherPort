...
def check_compressed_font_support(errors: list[str]) -> None:
    compressed_fonts = [
        path
        for path in (ROOT / "main").glob("lv_font_*.c")
    ...
    if compressed_fonts and "CONFIG_LV_USE_FONT_COMPRESSED=y" not in defaults:
        names = ", ".join(path.name for path in compressed_fonts)
        errors.append(
            "sdkconfig.defaults: CONFIG_LV_USE_FONT_COMPRESSED=y is required "
            f"by compressed device fonts: {names}"
        )
...
def main() -> int:
    ...
    check_compressed_font_support(errors)
